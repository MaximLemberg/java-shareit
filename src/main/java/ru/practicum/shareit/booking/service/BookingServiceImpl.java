...
public class BookingServiceImpl implements BookingService {
    ...
    public BookingDto add(BookingDtoAdd bookingDto, Long userId) {
        ...
        if (checkItem.getOwner().getId().equals(userId)) throw new EntityNotFoundException("Item Unavailable");
        if (!checkItem.getAvailable()) throw new NotAvailableException("Item Unavailable");
    ...
    public List<BookingDto> findAllById(Long bookerId, String state) {
        ...
        if (state.equalsIgnoreCase("WAITING")) {
            foundEntity.stream()
                    ...
                    .forEach(Booking -> found.add(bookingMapper.toDto(Booking)))
            ;
            return found;
        }
    ...
    public void checkTime(BookingDtoAdd bookingDto) throws ValidationException {
        if (bookingDto.getStart() == null) throw new ValidationException("Incorrect time");
        if (bookingDto.getEnd() == null) throw new ValidationException("Incorrect time");
        if (bookingDto.getEnd().isBefore(bookingDto.getStart())
                || bookingDto.getEnd().equals(bookingDto.getStart())
                || bookingDto.getStart().isBefore(LocalDateTime.now())) throw new ValidationException("Incorrect time");
